...
ll query(int si , int ss , int se , int qs , int qe)
{
	...
	return (query(2*si , ss , mid , qs , qe)%mod + query(2*si+1 , mid+1 , se , qs , qe)%mod)%mod;
}
...
void update(int si , int ss , int se , int qs , int qe , int val)
{
	...
	if(ss >= qs && se <= qe)
	{
		...
		if(ss != se)
		lazy[2*si] = (lazy[2*si]*val)%mod , lazy[2*si+1] =(lazy[2*si+1]* val)%mod;
		return;
	}
